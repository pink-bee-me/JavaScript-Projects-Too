...
const Calculator = {
    ...
    First_Operand: null,
    //this checks whether or not the sewcond operand has been input
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //we are saying that if the Display _Value does not contain a decimal 
    //point, then we want to add a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    //Claculator.First_Operand if it doesn't already exist
...
keys.addEventListener("click", (event) => {
    //the target variable is a n object that represents the element
    //that was clicked
    const { target } = event;
